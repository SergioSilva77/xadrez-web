...
try {
    function findElement(typeSelector, selector) {
        switch (typeSelector) {
            case 'class':
                el = document.getElementsByClassName(selector);
                el = (el.length <= 0) ? undefined : el
                break;
            case 'xpath':
    ...
    var el = findElement('selector', '.actions>button')
    if (!el) {
        return 'Botão "Ir para o Explore" não encontrado'
    }
    el.click()
    return 'sucesso'
} catch (error) {
...
function getElementByXpath(path) {
    return document.evaluate('/html/body/div[2]/ng-template/div/div[2]/bime-lock/div/div[2]/div/div[5]/div/div/div[2]/div[1]/div[4]/div/div[1]/i', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
  }